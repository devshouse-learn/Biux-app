...
Write-Host "📋 LIMPIEZA FINAL: Eliminando lib/data/ y validando migración" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "🔍 Verificando completitud de la migración..." -ForegroundColor Green
...
Write-Host "  ✅ Todas las features están migradas" -ForegroundColor Green
...
foreach ($file in $dartFiles) {
    ...
    if ($content -match "import\s+[`"']package:biux/data/") {
...
if ($problematicFiles.Count -gt 0) {
    ...
    Write-Host ""
    Write-Host "💡 Ejecuta los scripts de corrección de importaciones antes de continuar." -ForegroundColor Yellow
    exit 1
}

Write-Host "  ✅ No se encontraron importaciones problemáticas" -ForegroundColor Green
...
if (Test-Path "lib\data") {
    ...
    Write-Host "  ℹ️  lib/data ya no existe, no se requiere backup" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "⚠️  CONFIRMACIÓN REQUERIDA" -ForegroundColor Red
Write-Host "¿Estás seguro de que quieres eliminar lib/data/ permanentemente?" -ForegroundColor Yellow
Write-Host "Esta acción NO se puede deshacer (aunque tienes el backup)." -ForegroundColor Yellow
Write-Host ""
Write-Host "[S] Sí, eliminar lib/data/" -ForegroundColor Green
Write-Host "[N] No, cancelar limpieza" -ForegroundColor Red
Write-Host ""

$confirmation = Read-Host "Selecciona una opción (S/N)"
...
Write-Host ""
Write-Host "🗑️ Eliminando lib/data/..." -ForegroundColor Red
...
if (Test-Path "lib\data") {
    ...
    Write-Host "  ℹ️  lib/data/ ya había sido eliminado" -ForegroundColor Yellow
}

# PASO 7: Eliminar directorios vacíos en lib/
Write-Host ""
Write-Host "🧹 Limpiando directorios vacíos..." -ForegroundColor Green
...
if ($emptyDirs.Count -gt 0) {
    foreach ($emptyDir in $emptyDirs) {
        ...
        Write-Host "  ✅ Eliminado directorio vacío: $relativePath" -ForegroundColor Green
    }
} else {
    Write-Host "  ℹ️  No se encontraron directorios vacíos" -ForegroundColor Yellow
}
...
Write-Host "  📦 Modelos comunes en core: $(if ($coreModelsExist) { '✅' } else { '❌' })" -ForegroundColor White

# Verificar que lib/data ya no existe
$dataExists = Test-Path "lib\data"
Write-Host "  🗑️ lib/data/ eliminado: $(if (-not $dataExists) { '✅' } else { '❌' })" -ForegroundColor White
...
Write-Host ""
Write-Host "🚀 VALIDACIÓN RECOMENDADA:" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "🎯 ¡MIGRACIÓN CLEAN ARCHITECTURE COMPLETADA!" -ForegroundColor Green
