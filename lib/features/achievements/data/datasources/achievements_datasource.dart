...
class AchievementsDatasource {
  ...
  Future<void> saveAchievements(String userId, Map<String, dynamic> data) async {
    await _firestore.collection('achievements').doc(userId).set(data, SetOptions(merge: true));
  }
  ...
  Future<void> updateProgress(String userId, String achievementId, double value) async {
    await _firestore.collection('achievements').doc(userId).set({
      achievementId: {
        'currentValue': value,
      },
