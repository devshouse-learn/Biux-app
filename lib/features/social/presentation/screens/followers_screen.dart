...
class FollowersScreen extends StatefulWidget {
  ...
  const FollowersScreen({Key? key, required this.userId, this.showFollowers = true}) : super(key: key);
  @override
...
class _FollowersScreenState extends State<FollowersScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _tabCtrl = TabController(length: 2, vsync: this, initialIndex: widget.showFollowers ? 0 : 1);
  }
        ...
        if (!snap.hasData || snap.data!.docs.isEmpty) {
          return Center(
            child: Column(
              ...
              children: [
                Icon(
                  collection == 'followers' ? Icons.people_outline : Icons.person_add_alt,
                  size: 64,
                  color: Colors.grey[300],
                ),
                const SizedBox(height: 16),
                Text(
                  collection == 'followers' ? 'Sin seguidores aún' : 'No sigues a nadie aún',
