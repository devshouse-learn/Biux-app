...
class RideTrackerProvider with ChangeNotifier {
  ...
  Future<void> initLivePosition() async {
    ...
    try {
      ...
      AppLogger.error(
        'initLivePosition error',
        error: e,
        tag: 'RideTrackerProvider',
      );
    }
  }
  ...
  Future<bool> stopAndSave(String userId, String name) async {
    ...
    if (isOnline) {
      try {
        ...
        AppLogger.warning(
          'Error Firestore, guardando offline',
          error: e,
          tag: 'RideTrackerProvider',
        );
        return _saveOffline(
  ...
  Future<bool> _saveOffline(
    ...
    try {
      ...
      _resetTrackingState();
      AppLogger.debug(
        'Rodada guardada offline: ${offlineRide.id}',
        tag: 'RideTrackerProvider',
      );
      return true;
    } on FirebaseException catch (e) {
      AppLogger.error(
        'Error guardando offline',
        error: e,
        tag: 'RideTrackerProvider',
      );
      _isSaving = false;
  ...
  Future<int> syncPendingRides(String userId) async {
    ...
    for (final ride in pending) {
      try {
        ...
        synced++;
        AppLogger.debug(
          'Rodada sincronizada: ${ride.id} -> $trackId',
          tag: 'RideTrackerProvider',
        );
      } on FirebaseException catch (e) {
        AppLogger.error(
          'Error sincronizando ${ride.id}',
          error: e,
          tag: 'RideTrackerProvider',
        );
      }
    ...
    Future(() async {
      try {
        if (savedPoints.isNotEmpty) {
          ...
          AppLogger.debug(
            '[BG] Puntos GPS guardados: ${savedPoints.length}',
            tag: 'RideTrackerProvider',
          );
        }
      } on FirebaseException catch (e) {
        AppLogger.error(
          '[BG] Error guardando puntos',
          error: e,
          tag: 'RideTrackerProvider',
        );
      }
      ...
      try {
        ...
        AppLogger.debug(
          '[BG] Stats y logros actualizados',
          tag: 'RideTrackerProvider',
        );
      } on FirebaseException catch (e) {
        AppLogger.error(
          '[BG] Error actualizando stats',
          error: e,
          tag: 'RideTrackerProvider',
        );
      }
  ...
  Future<void> _updateAchievements(
    ...
    try {
      ...
      AppLogger.error('Error', error: e, tag: 'RideTrackerProvider');
    }
  ...
  Future<void> loadHistory(String userId) async {
    ...
    try {
      ...
      try {
        ...
        AppLogger.error(
          'Error loading history (fallback)',
          error: e2,
          tag: 'RideTrackerProvider',
        );
      }
