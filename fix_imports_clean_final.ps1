...
foreach ($file in $dartFiles) {
    ...
    try {
        ...
        if ($originalContent -ne $content) {
            ...
            $modifiedFiles++
            Write-Host "  [OK] Corregido: $relativePath" -ForegroundColor Green
        }
        
    } catch {
        Write-Host "[ERROR] Error procesando $relativePath : $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
if ($modifiedFiles -gt 0) {
    ...
    Write-Host ""
    Write-Host "No se encontraron importaciones que corregir." -ForegroundColor Blue
}